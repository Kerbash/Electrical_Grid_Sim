...
int main() {
    ...
    // allocate do pinned memory on device
    double* d_inputMatrix;
    double* d_outputMatrix;
    cudaMallocHost((void**)&d_inputMatrix, INPUT_SIZE * INPUT_SIZE * sizeof(double));
    cudaMallocHost((void**)&d_outputMatrix, INPUT_SIZE / TOTAL_POOL * INPUT_SIZE / TOTAL_POOL * sizeof(double));

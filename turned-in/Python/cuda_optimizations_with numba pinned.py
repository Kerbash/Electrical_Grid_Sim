# ---------------------------------------------------------------------------
# cuda_optimization_with numba pinned.py
...
def cudaPool(input, temp, output, last_pool):
    ...
    # sum of two values
    shared[cuda.threadIdx.x] = input[x1 * INPUT_SIZE + y1] + input[x2 * INPUT_SIZE + y2]
    cuda.threadfence()
    ...
    if cuda.threadIdx.x == 0:
        if last_pool: # if this is the last pool, save the sum to output
            output[cuda.blockIdx.x * cuda.gridDim.y + cuda.blockIdx.y] = shared[0] + shared[1]
        else: # if this is not the last pool, save the sum to input
            temp[cuda.blockIdx.x * int(INPUT_SIZE / POOL_SIZE) + cuda.blockIdx.y] = shared[0] + shared[1]
...
# create output array
output = np.zeros(int((INPUT_SIZE / TOTAL_POOL) * INPUT_SIZE / TOTAL_POOL), dtype=np.float64)
temp = np.zeros(int((INPUT_SIZE / POOL_SIZE) * INPUT_SIZE / POOL_SIZE), dtype=np.float64)

# allocate pin memory on the device
d_input = cuda.pinned_array(INPUT_SIZE * INPUT_SIZE, dtype=np.float64)
d_output = cuda.pinned_array(int((INPUT_SIZE / TOTAL_POOL) * INPUT_SIZE / TOTAL_POOL), dtype=np.float64) # copy=False because it's just an empty array
d_temp = cuda.pinned_array(int((INPUT_SIZE / POOL_SIZE) * INPUT_SIZE / POOL_SIZE), dtype=np.float64) # copy=False

# start timer
total_time = time.perf_counter()

# reshape the input array to 1d
input = input.reshape(INPUT_SIZE * INPUT_SIZE)
# copy to pinned memory
np.copyto(d_input, input)
...
output = output.reshape(int(INPUT_SIZE / TOTAL_POOL), int(INPUT_SIZE / TOTAL_POOL))
